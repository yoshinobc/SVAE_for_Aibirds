    ...
    def denoiseInput(self, t, noise_rate=0.4):
        ...
        if noise_rate > 0.0:
            for t_i, t_e in enumerate(t):
                ind_arr = [t_i for t_i in range(1, min(len(t_e), 30*93))]
    ...
    def predict(self, batch_size, randFlag):
        ...
        for _ in (range(30*94)):
            ...
            t = [self.embed(xp.array([t_each], dtype=xp.int32))
                 for t_each in t]
            count_len += 1
        name_arr_arr = list(name_arr_arr)
        if self.gpu >= 0:
            global xp
            import numpy as xp
