...
def train(args, encdec, model_name_base):   
    ...
    for e_i in range(encdec.epoch_now, args.epoch):
        ...
        if e_i % 5 == 0:
            ...
            i = 0
            for tupl in encdec.getBatchGen(args, False, args.predict_size):
                mu_arr, var_arr = encdec.encode(tupl)
                z = F.gaussian(mu_arr[0], var_arr[0])
                tenti = encdec.predict(args.predict_size, z=z)
            ...
            i = 0
            for tupl in encdec.getBatchGen_test(args, False, args.predict_size):
                mu_arr, var_arr = encdec.encode(tupl)
                z = F.gaussian(mu_arr[0], var_arr[0])
                tenti = encdec.predict(args.predict_size, z=z)
                encdec.dec.reset_state()
